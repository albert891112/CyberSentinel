...
def load_phishtank_data(path: str = "data/Malicious_url/verified_online.csv", 
    ...
    try:
        if not os.path.exists(path):
            print(f"    [!] PhishTank not found at {path}")
            return None
...
def load_fireeye_data(path: str = "data/fireeye_urls.csv") -> tuple[list[str], list[str]] | None:
    """Load FireEye combined dataset with fallback."""
    try:
        if not os.path.exists(path):
...
def load_all_data(malicious_sample: int = 100000, 
                  benign_sample: int = 100000) -> tuple[list[str], list[str], set[str]] | None:
    ...
    if fireeye_result:
        ...
        print("    [i] FireEye data missing, increasing samples from other sources...")
...
def _process_chunk(chunk_urls: list[str], 
                   top_domains: set[str], 
                   trigram_extractor: TrigramFeatureExtractor) -> list[dict]:
...
def extract_features_parallel(urls: list[str], 
                              trigram_extractor: TrigramFeatureExtractor,
                              top_domains: set[str],
                              fit_trigrams: bool = False) -> pd.DataFrame:
    """
    ...
    func = partial(_process_chunk, top_domains=top_domains, trigram_extractor=trigram_extractor)
    ...
    if n_workers > 1:
        with ProcessPoolExecutor(max_workers=n_workers) as executor:
            for i, chunk_res in enumerate(executor.map(func, chunks)):
                ...
                if (i + 1) % 2 == 0:
                     print(f"      Processed {min((i+1)*chunk_size, len(urls))}/{len(urls)}...")
    else:
